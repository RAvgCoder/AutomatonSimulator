...
use crate::parser::{Parser, ParserError, State};
use crate::parser::parser::ParserError::{
    MissingObjSeparator, NoObjName, ObjNameMismatch, ObjNameNotFound,
    ObjNameOverFlow, ObjNameSyntaxErr,
};
...
impl Parser {
    ...
    pub fn parse<'a>(program: String) -> Option<Automaton<'a>> {
        ...
        while parser.can_consume() {
            ...
            match user_state {
                State::Type => {
                    let automaton = parser.try_consume_name()
                        .unwrap()
                        .to_ascii_uppercase();
        ...
        assert_eq!(parser.program_iter.len(), 0, 
    ...
    fn extract_obj_state(parser: &mut Parser, obj_name_state: &mut Iter<State>) -> State {
        ...
        let user_state = State::from_string(&obj_name)
            .expect(&format!("{}", ObjNameNotFound(format!(
                "Object name found is not a recognisable. Object name found is '{}'", obj_name
            ))));
        ...
        assert_eq!(user_state, *parsers_expected_state, "{}", ObjNameMismatch(format!(
...
Found: {:?}",
    ...
    fn try_consume_name(&mut self) -> Result<String, ParserError> {
        ...
        let first_char = prog_iter.next()
            .ok_or(OutOfInput("Tried to read object name but no input is found".to_string()))?;
    ...
    fn consume_separator(&mut self, separator: Separator) -> Result<(), ParserError> {
        ...
        Err(MissingObjSeparator(
            format!("Missing separator {:?} found at prog_index {}", separator, self.cursor)
        ))
    }
    fn consume_scope(&mut self, scope: Scope) -> Result<Parser, ParserError> {
        ...
        // Tries to consume the starting quotation marks
        let opening_scope_char = prog_iter.next()
            .ok_or(ScopeError("Tried to parse scope, but no input was found".to_string()))?;
        ...
        if inner_scope_content.len() == (remaining_prog_len - 1) // Missing end scope 
        {
...
fn prog_preprocessor(program: String) -> String {
    ...
    let mut prog = binding
        .chars();

    assert_eq!(prog.nth(0), Some('{'), "Program doesnt start with opening curly braces");
    assert_eq!(prog.last(), Some('}'), "Program doesnt emd with closing curly braces");
...
impl State {
    fn from_string(s: &str) -> Option<State> {
